...
if "target_data" not in st.session_state:
    ...
    # assemble plot data
    plate_layout = pd.DataFrame(construct_design.generate_96_platemap(), columns=["Plate_well"])
    plate_layout["row"] = plate_layout.Plate_well.str[:1]
    plate_layout["column"] = plate_layout.Plate_well.str[1:].astype(str)  # Keep as string throughout
    ...
    construct_plate_layout = figure(
            ...
            x_range=FactorRange(factors=list(map(str, sorted(plate_layout["column"].unique())))),
            y_range=FactorRange(factors=list(reversed(sorted(plate_layout["row"].unique())))),  # Reverse rows to match plate convention
        )
    ...
    hover = HoverTool(tooltips=[("Plate well", "@Plate_well"), ("Construct name", "@Construct_name")])
